...
<head>
    ...
    <style>
        ...
        .product-gallery{position:relative}.product-gallery .gallery-image{position:absolute;inset:0;opacity:0;transition:opacity .32s ease}.product-gallery .gallery-image.is-active{opacity:1}.gallery-dots{position:absolute;z-index:1;bottom:9px;left:0;right:0;display:flex;justify-content:center;gap:5px;pointer-events:none}.gallery-dot{width:6px;height:6px;border-radius:50%;background:#b8c9c1;box-shadow:0 1px 3px #18332c33}.gallery-dot.is-active{width:15px;border-radius:5px;background:var(--green)}
    ...
    <main class="wrap">
        ...
        <div class="photo product-gallery" data-product-gallery>
            ...
            @endif
                        ...
                        <div class="cardbody"><div class="maker">{{ $product->manufacturer ?: 'MediServe' }}</div><div class="pname">{{ $product->name }}</div><div class="pack">{{ $product->packaging ?: ($product->composition ? \Illuminate\Support\Str::limit($product->composition, 36) : 'Healthcare essential') }}</div>
