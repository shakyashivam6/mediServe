<x-layouts.admin-layout title="Edit Captain">

    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title mb-3">Edit Captain</h4>

                    <form method="POST" action="{{ route('admin.captains.update', $user) }}">
                        @csrf
                        @method('PUT')

                        <h5 class="fs-14 text-uppercase text-muted mb-2">Login Details</h5>
                        @include('Admin.partials.person-fields', ['user' => $user, 'locations' => $locations])
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="store_id" id="store_id" class="form-select @error('store_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}" @selected(old('store_id', $user->store_id) == $store->id)>{{ $store->store->shop_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="vehicle_type" id="vehicle_type" class="form-select @error('vehicle_type') is-invalid @enderror">
                                    ...
                                    @foreach ($vehicleTypes as $type)
                                        <option value="{{ $type }}" @selected(old('vehicle_type', $user->vehicle_type) === $type)>{{ ucfirst($type) }}</option>
                                    @endforeach
                                </select>
